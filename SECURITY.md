...
- Build the web container from `web/Dockerfile` so Next.js dependencies are preinstalled and the runtime executes as the non-root `node` user; the backup image likewise carries its Restic tooling at build time so production jobs do not run package managers as root.

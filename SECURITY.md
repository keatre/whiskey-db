...
- Build the flagship image from the repository root (`Dockerfile`) so FastAPI, Next.js, and the backup scheduler install their dependencies during the image build and runtime only launches the three processes under Tini (no `pip`/`npm` invocations as root on each boot).
- When exposing the app globally, terminate TLS upstream (e.g., Cloudflare Tunnel) and retain `COOKIE_SECURE=auto`; consider Cloudflare Access or another identity-aware proxy on `/admin` while keeping LAN-only installs on plain HTTP when desired. Requests routed through Cloudflare are always treated as external (authentication required) even if `ALLOW_LAN_GUEST=true`, so only direct LAN access receives guest privileges (the web proxy adds `x-whiskey-via=cloudflare` automatically). Use `LAN_GUEST_HOSTS` to enumerate which hostnames should still be treated as LAN for guest access. The unified image bakes `NEXT_PUBLIC_API_BASE`/`NEXT_BACKEND_ORIGIN` at build time, so set those env vars before rebuilding (e.g., `docker compose build whiskey`) when deploying behind different URLs. A new `lan_guest_decision` log line (INFO), response header (`X-Whiskey-Lan-Decision`), and in-memory counter in `app.deps` can be inspected whenever you need to audit why a request was accepted or denied as a LAN guest.

...
### Fixed
- Remediated newly disclosed frontend and backend dependency advisories by upgrading Next.js, PostCSS, Sharp, `brace-expansion`, `python-multipart`, Pillow, and `pillow-heif`; refreshed compatible framework and tooling dependencies; and updated WebAuthn binary decoding for current TypeScript DOM types (`web/package.json`, `web/package-lock.json`, `api/requirements.txt`, `web/src/app/security/page.tsx`, `web/src/app/signin/_LoginClient.tsx`).
...
- Starter Grafana dashboard exports for logbox (stored under `dashboard/`, ignored by git).
...
- README now documents modules and the Wine database setting (`README.md`).

...
> Logging helpers ship with the image; do **not** bind-mount `ops/logging` unless you intentionally maintain your own wrapper scripts, otherwise `/logging/with-logging.sh` will be missing on hosts that only keep `/opt/appdata/whiskey-db`. Mount points such as `/data` and `/logs` are created automatically at container start (and chowned to `PUID`/`PGID` when set), and the compose files now request Docker to `create_host_path` for those binds so you no longer have to pre-create the host folders.
...
- Defaults bind mounts to `/opt/appdata/whiskey-db` but honors `WHISKEY_HOST_ROOT`, `WHISKEY_ENV_FILE`, and `WHISKEY_COMPOSE_FILE_PATH` overrides.
- Mounts your host `.env` and compose file into `/config` so backups can capture them when `BACKUP_LOCAL_FILES=true`.
Because `docker-compose.yml` is now gitignored, you can drop that copied file next to the repo without Git trying to stage it.

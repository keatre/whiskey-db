...
export default function NewBottlePage() {
  ...
  const [form, setForm] = useState({
    ...
    mashbill_markdown: '',
    notes_markdown: '',
    image_url: ''   // will be filled by upload response
  });
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      set('image_url', data.url);     // store the local URL: /static/uploads/...
      setPreviewUrl(`${API}${data.url.startsWith('/') ? '' : '/'}${data.url}`); // absolute for preview
    } catch (err: any) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const payload: any = {
      ...
      image_url: form.image_url || null      // LOCAL url returned by API
    };
    ...
    // auto-fill ABV if only Proof is provided
    if (payload.proof != null && payload.abv == null) {
      payload.abv = Math.round((payload.proof / 2) * 10) / 10; // e.g., 100 → 50.0
    }
  ...
  return (
    <main>
      <h1>New Bottle</h1>
      <form onSubmit={submit} style={{display:'grid', gridTemplateColumns:'220px 420px', gap:10, alignItems:'center', maxWidth:'980px'}}>
        <label>Brand</label>
        <input
        ...
        <input type="number" inputMode="numeric" min="1900" max="2100" step="1"
        ...
        <div>
          ...
          <div style={{marginTop:8}}>
            ...
            {!previewUrl && !uploading && <div style={{fontSize:12, opacity:0.7}}>You can leave this blank for now.</div>}
          </div>
        </div>
